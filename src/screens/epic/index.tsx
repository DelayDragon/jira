...
export const EpicScreen = () => {
    ...
    return <ScreenContainer>
        ...
        <List style={{overflow:'auto'}} dataSource={epics} itemLayout={'vertical'} renderItem={epic => <List.Item>
            ...
            <div>
                {
                    tasks?.filter(
                        task => task.epicId === epic.id).map(
                            task => <Link to={`projects/${currentProject?.id}/kanban?editingTaskId=${task.id}`} key={task.id}>{task.name}</Link>)
                }
            </div>
        </List.Item>}></List>
        <CreateEpic onClose={() => setEpicCreateOpen(false)} open={epicCreateOpen}></CreateEpic>
